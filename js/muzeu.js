...
/* ---- contribuire: overlay peste hartă, 5 poze via ImgBB, ★ = principala ---- */
const IMGBB_KEY = '3446f2ea34c2e93a787d089e934b2d7c';
const IMGBB_MAX_FISIER = 5 * 1024 * 1024; /* 5MB / poză (plan gratuit) */
const IMGBB_MAX_POZE = 5;
...
/* Upload o poză la ImgBB → URL direct (https://) */
async function uploadLaImgBB(file) {
  if(!file) return null;
  if(file.size > IMGBB_MAX_FISIER){
    toastPoarta('„' + (file.name || 'imagine') + '” depășește 5MB și a fost sărită.');
    return null;
  }
  ...
  try{
    ...
    if(data && data.success && url) return String(url);
    const msg = (data && data.error && data.error.message) ? data.error.message : 'răspuns invalid';
    toastPoarta('ImgBB a refuzat o imagine: ' + msg);
    return null;
  }catch(e){
    toastPoarta('Eroare de rețea la ImgBB. Verifică netul și reîncearcă.');
    return null;
  }
}
...
function randeazaPreviewContribuie(){
  ...
  counterEl.textContent = stareContribuie.fisiere.length + '/' + IMGBB_MAX_POZE + ' imagini alese';
}
...
function onFisiereAlese(list){
  ...
  const spatiu = IMGBB_MAX_POZE - stareContribuie.fisiere.length;
  ...
  deAdaugat.forEach(f => {
    if(f.size > IMGBB_MAX_FISIER){ toastPoarta('„' + (f.name || 'imagine') + '” depășește 5MB.'); return; }
...
async function trimiteContribuie(e){
  ...
  try{
    const total = stareContribuie.fisiere.length;
    const urls = [];
    for(let k = 0; k < total; k++){
      setStareContribuie('Se încarcă imaginea ' + (k + 1) + ' din ' + total + ' pe ImgBB…');
      const url = await uploadLaImgBB(stareContribuie.fisiere[k]);
      if(!url){
        setStareContribuie('Încărcarea pe ImgBB a eșuat. Reîncearcă.', true);
        stareContribuie.trimitere = false;
        if(btn) btn.disabled = false;
        return;
      }
    ...
    /* ★ = principala (default prima); restul în ordine → img_1..img_4, gol = '-' */
    const idxP = Math.min(stareContribuie.indexPrincipal || 0, urls.length - 1);
    const principala = urls[idxP];
    const rest = urls.filter((_, k) => k !== idxP);
    while(rest.length < 4) rest.push('-');
    const payload = {
      titlu: titlu,
      descriere_scurta: descriere.slice(0, 180),
      poveste: descriere,
      material: material,
      an: an,
      perioada: perioada,
      ...
      autor: autor,
      locatie_fizica: locatie,
      imagine_principala: principala,
      img_1: rest[0] || '-',
      img_2: rest[1] || '-',
      img_3: rest[2] || '-',
      img_4: rest[3] || '-',
      sala_id: (typeof MUZEU !== 'undefined' && MUZEU.sali && MUZEU.sali[0]) ? MUZEU.sali[0].id : 'sala-documentelor',
      raion_id: (typeof raionCurent !== 'undefined' && raionCurent) ? raionCurent : 'chisinau',
      sat_id: (typeof satCurent !== 'undefined' && satCurent) ? satCurent : ''
    };
    const deployUrl = window.DEPLOY_URL;
    if(!deployUrl || /PASTE_AICI/.test(deployUrl)){
      setStareContribuie('Lipsește DEPLOY_URL pentru trimitere.', true);
      stareContribuie.trimitere = false;
      if(btn) btn.disabled = false;
      return;
    }
    setStareContribuie('Se trimite contribuția spre arhivă…');
    const resp = await fetch(deployUrl, {
      method: 'POST',
      headers: { 'Content-Type': 'text/plain;charset=utf-8' },
      body: JSON.stringify({ action: 'propuneDosar', payload: payload })
    });
    const result = await resp.json().catch(() => null);
    if(result && (result.ok || result.success)){
      const idNou = result.ID_auto || result.id || '';
      setStareContribuie('Mulțumim! Propunerea ' + (idNou ? idNou + ' ' : '') + 'a ajuns la muzeu și așteaptă validarea.');
      toastPoarta('Contribuția a fost trimisă. Mulțumim!');
      curataFormContribuie();
      setTimeout(() => inchideContribuie(true), 1800);
    }else{
      const msg = 'Nu s-a putut salva: ' + ((result && result.error) || 'eroare necunoscută');
      setStareContribuie(msg, true);
      toastPoarta(msg);
    }
  }catch(err){
    setStareContribuie('Eroare de rețea la trimitere. Reîncearcă.', true);
    toastPoarta('Eroare de rețea la trimitere');
  }finally{
    stareContribuie.trimitere = false;
    if(btn) btn.disabled = false;
  }
