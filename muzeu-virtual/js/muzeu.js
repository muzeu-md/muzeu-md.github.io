...
function ramaPentru(o){
  const fix = cadruIdDe(o);
  if(fix) return fix; /* ID atribuit la creare (Pages CMS) */
  return (hashId(o.id) % NR_RAME) + 1; /* compatibilitate: fișe vechi */
}
/* Cele 8 stiluri vizuale de ramă: ID unic + nume + indiciu (legendă completă). */
const CADRE = {
  1: {nume: 'auriu clasic', hint: 'Potrivit actelor și diplomelor.'},
  2: {nume: 'auriu cu ornamente', hint: 'Piese festive, costume.'},
  3: {nume: 'lemn închis', hint: 'Fotografii, mobilier, unelte.'},
  4: {nume: 'negru modern', hint: 'Aparate, tehnică.'},
  5: {nume: 'vișiniu', hint: 'Mărturii, scrisori, medalii.'},
  6: {nume: 'bronz', hint: 'Ceramică, metal, gospodărie.'},
  7: {nume: 'argintiu', hint: 'Natură, minerale, instrumente.'},
  8: {nume: 'auriu deschis', hint: 'Țesături, broderii.'}
};
function cadruIdDe(o){ const n = +(o && o.cadruId); return (n >= 1 && n <= NR_RAME) ? n : null; }
function cadruText(o){
  const n = ramaPentru(o), c = CADRE[n];
  return n + ' - cadru ' + c.nume + ' · ' + c.hint;
}
function randareLegendaCadre(){
  const ul = document.getElementById('f-cadre-lista');
  if(!ul || ul.childElementCount) return;
  ul.innerHTML = Object.keys(CADRE).map(k =>
    `<li><strong>${k} - cadru ${CADRE[k].nume}</strong> · ${CADRE[k].hint}</li>`
  ).join('');
}
